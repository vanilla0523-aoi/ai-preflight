...
**▶ デモ： https://ai-preflight-coral.vercel.app**
（画面の「サンプルを入力」→「送信前チェック」で一連の動作を確認できます。入力内容はブラウザ内で処理され、サーバーへは送信されません）
...
- **デモURL:** https://ai-preflight-coral.vercel.app （Vercelで公開。ローカル起動は下記[セットアップ](#5-セットアップ)参照）
- **ホスティング:** 本アプリはサーバー処理を持たないため、入力文がサーバーへ届かないことは配信形態のレベルで成立している
- **デモシナリオ:**
...
- **実装済み:** テキスト入力／送信前チェック／Email・API Key・AWS Key・電話番号・IPv4・クレジットカード番号（Luhn検証）・氏名らしき情報の検出／社内NGワード検出（設定ファイル）／Risk Level・Risk Score／検出理由表示／該当箇所ハイライト／Safe Prompt生成・コピー／サンプル入力／編集後の再チェック促し／ユニットテスト（vitest・20ケース）
...
src/lib/security/
  ...
  scanner.test.ts          # ユニットテスト（デモシナリオ・マージ・誤検知抑制・結果の安定性）
...
- **ブラウザ機能経由で外部へ出る経路も塞ぐ** — 自前のコードで送信しないだけでは不十分だと考え、入力欄に `spellCheck={false}` を設定した。一部ブラウザの拡張スペルチェックは入力文をベンダーのサーバーへ送信するため、ここを塞がないと「外部送信ゼロ」が成立しない。あわせて `autoComplete="off"` 等で入力履歴として端末に残るのを防ぎ、Grammarly・パスワードマネージャ向けの除外属性も付与している
- **古いチェック結果を「安全」と誤認させない** — チェック後に文章を編集すると結果を古い状態として薄く表示し、再チェックを促す。修正したつもりで未チェックのまま送信される事故を防ぐための挙動

#### クライアント完結の設計とトレードオフ

検知をブラウザ内で完結させたことで「入力文がサーバーへ届かない」ことは配信形態のレベルで保証される（本PoCはサーバー処理を持たない静的サイトとして配信できる）。一方で、**社内NGルール（`organizationRules.ts`）もクライアントへ配信されるため、NGワードの一覧は閲覧者から読み取れる**。本PoCのルールはデモ用のダミーなので問題にならないが、「未公開プロジェクト名の一覧」自体が機密になる実運用では、この構成は成立しない。

実運用でこのトレードオフを解く場合は、次のいずれかになると考えている。

- 社内ネットワーク内への配置やブラウザ拡張として配布し、ルールの配信範囲を組織内に閉じる
- ルール判定のみを自社サーバー側で行う（この場合、入力文の扱いを設計し直す必要がある）
- ルールをハッシュ化して保持し、原文を復元できない形で照合する（完全一致に限られる）
...
- **設計を見直した点:** 自分のコードで外部送信しないことばかり見ていて、ブラウザ自身が入力を外へ送りうる経路（拡張スペルチェック）を見落としていた。中核方針に関わるため入力欄の属性で塞いだ。また、社内NGルールは企業側が設定ファイルを編集する前提なので、空文字キーワードで無限ループするなど「設定ミスでアプリが壊れない」ことを検証対象に加えた
- **既知の制約:** 検知は正規表現＋キーワード一致の範囲。文脈依存の機密情報（未公開の経営情報など）は検出できない／電話番号はハイフン区切り形式のみ対象（誤検知抑制のため）／ブラウザ拡張が入力欄を読み取る可能性は属性で自衛しているが完全には制御できない
